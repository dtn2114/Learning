...
def momentum(price, n, chart=False):
    ...
    if chart:
        ...
        price_norm = price.drop('SPY', axis=1)
        ...
        f = df_temp.plot(title = "Momentum vs. Vol",ax=f, linewidth=0.7)
        f.set_ylabel("Momentum, Volatility")
        f.set_xlabel("Date")
        f.legend(loc='best', prop={'size':5})
        plt.grid(True)
        f2 = fig.add_subplot(2,1,2)
        price_norm.plot(title = "Price", ax=f2, color='r', linewidth=0.7)
        f2.set_ylabel("Price")
        f2.set_xlabel("Date")
        plt.grid(True)
        f2.legend(loc='best', prop={'size':5})
        fig.tight_layout()
        fig.savefig("Momentum_Volatility")
    return mom
